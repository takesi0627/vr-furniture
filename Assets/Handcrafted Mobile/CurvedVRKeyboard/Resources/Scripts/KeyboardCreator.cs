...
namespace CurvedVRKeyboard {
    ...
    public class KeyboardCreator : KeyboardComponent {
        ...
        [SerializeField] float defaultSpacingColumns = 60.3f;
        [SerializeField] float defaultSpacingRows = 1.0f;
        ...
        private const int spaceKeyNumber = 29;
        ...
        public void ManageKeys() {
            ...
            if (!errorReporter.IsErrorPresent()) {
                if (centerPointDistance.Equals(-1f)) {
                    CurvatureToDistance();
                }
                FillAndPlaceKeys();
            }
        ...
        private void PositionSingleLetter(KeyboardItem key)
        {
            ...
            key.transform.localPosition = CalculatePositionOnPlane(iteration - keysPlaced, key);
        ...
        /// <summary>
        /// Calculates the position on plane.
        /// </summary>
        /// <returns>The position on plane.</returns>
        /// <param name="placed">The number of where the key is from the left(0).</param>
        public Vector3 CalculatePositionOnPlane (float placed, KeyboardItem key) {
            float x = 0.0f;
            float y = -row * RowSpacing;
            float z = 0.0f;

            x = (-(lettersInRowsCount[row] / 2.0f - 0.5f) + placed) * ColumnSpacing; 
            if ((row == 1 || row == 2) && placed.Equals(0)) {
                x -= 0.25f * ColumnSpacing;
                key.FillMaterial(1.5f);
            } else if ((row == 1 || row == 2) && (placed.Equals(lettersInRowsCount[row] - 1))) {
                x += 0.25f * ColumnSpacing;
                key.FillMaterial(1.5f);
            } else if (row == 3 && placed.Equals(0)) {
                x -= 1.25f * ColumnSpacing;
                key.FillMaterial(3.0f);
            } else if (row == 3 && placed.Equals(lettersInRowsCount[row] - 1)) {
                x += 1.25f * ColumnSpacing;
                key.FillMaterial(3.0f);
            }
        ...
        public void ChangeMaterialOnKeys () {
            foreach(KeyboardItem key in keys) {
                ...
                key.SetFonts(TextFont, TextColor);
            }
        }
        ...
        public float Curvature {
            ...
            set {
                if(!curvature.Equals( 1f - value )) {// Value changed
                    curvature = 1f - value;
                    CurvatureToDistance();
                    ManageKeys();
                    space.ManipulateSpace(this, spaceSprite);
        ...
        public float RowSpacing {
            ...
            set {
                if (!defaultSpacingRows.Equals(value)) {
                    defaultSpacingRows = value;
                    CurvatureToDistance();
                    ManageKeys();
                    space.ManipulateSpace(this, spaceSprite);
                }
        ...
        public float ColumnSpacing {
            get {
                return defaultSpacingColumns;
            }
            set {
                if (!defaultSpacingColumns.Equals(value)) {
                    defaultSpacingColumns = value;
                    CurvatureToDistance();
                    ManageKeys();
                    space.ManipulateSpace(this, spaceSprite);
                }
